...
class CheckCommand {
  ...
  static void checkDependencies(Params params) {
    ...
    for (final dependency in params.dependencies) {
      ...
      if (lockDependencies.isEmpty) throw ArgumentError('${dependency.name} is not yet included in the pubspec.lock. Make sure you run pacakges get');
    ...
    if (dependencyMismatches.isNotEmpty) {
      final message = 'We found some dependency mismatches: ${dependencyMismatches.length}';
      Logger.logInfo(message);
